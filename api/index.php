...
try {
    ...
    if ($resource === 'media' && $method === 'POST') {
        ...
        respond(['filename' => $safeName, 'public_url' => $url, 'storage_location' => rtrim($config['app']['uploads_url'] ?? 'storage/uploads', '/'), 'mime_type' => $mime, 'size_bytes' => (int) $file['size'], 'usage_count' => 0]);
    }
