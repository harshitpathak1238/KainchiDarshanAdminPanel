...
6. Open `/admin/` and enter one of the three allowlisted admin emails configured in `api/lib.php`. No password is used by this login flow. Each admin section is available as its own page, including `/admin/blog.html`, `/admin/blog-new.html`, and `/admin/blog-edit.html?id=POST_ID`.
...
The admin frontend uses shared runtime code and styles, with separate HTML entry pages for each section. Navigation loads the relevant page instead of rendering every section into one document; the old `/admin/#blog` hash remains supported as a legacy entry point.
