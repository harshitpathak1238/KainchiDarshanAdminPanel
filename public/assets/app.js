...
async function blogPage() {
  const content = document.querySelector('#content');
  content.classList.remove('content-blog-editor');
  const toolbar = `
...
function initializeRichEditor(wrapper) {
  ...
  const selectImage = image => {
    wrapper.querySelectorAll('img.editor-image-selected').forEach(item => item.classList.remove('editor-image-selected'));
    if (image) image.classList.add('editor-image-selected');
  };
  const resizeSelectedImage = () => {
    const image = wrapper.querySelector('img.editor-image-selected');
    if (!image) return toast('Click an image first, then choose Resize image.');
    const currentWidth = image.getAttribute('width') || image.naturalWidth || '';
    const width = prompt('Image width in pixels (leave blank to use the original size)', currentWidth);
    if (width === null) return;
    if (width.trim() === '') {
      image.removeAttribute('width'); image.removeAttribute('height');
    } else if (/^\d+$/.test(width.trim()) && Number(width) > 0) {
      const nextWidth = Number(width); const ratio = image.naturalWidth && image.naturalHeight ? image.naturalHeight / image.naturalWidth : 0;
      image.setAttribute('width', String(nextWidth));
      if (ratio) image.setAttribute('height', String(Math.round(nextWidth * ratio)));
    } else return toast('Enter a positive whole number for the image width.');
    syncSource();
  };
  ...
  const runCommand = async command => {
    ...
    else if (command === 'image') await openMediaPicker(asset => { const promptedAlt = prompt('Alt text for this image', asset.filename || ''); const alt = (promptedAlt || asset.filename || 'Image').trim(); insertRichHtml(editor, `<img class="responsive-image" src="${escapeHtml(asset.public_url)}" alt="${escapeHtml(alt)}">`); });
  ...
  editor.visual.addEventListener('click', event => { if (event.target.matches('img')) { selectImage(event.target); rememberRange(); } else selectImage(null); });
  ...
  wrapper.querySelector('[data-editor-resize]')?.addEventListener('mousedown', event => event.preventDefault());
  wrapper.querySelector('[data-editor-resize]')?.addEventListener('click', resizeSelectedImage);
...
async function blogForm(id = '') {
  ...
  editor.querySelectorAll('[required]').forEach(field => field.removeAttribute('required'));
  ...
  const savePost = async () => {
    ...
    try {
      ...
      if (error.fieldErrors?.slug) {
        ...
        toast(`Blog post save failed: ${error.fieldErrors.slug}`);
      } else {
...
async function mediaPage() {
  const content = document.querySelector('#content');
  content.innerHTML = pageHeading('Content', 'Media Library', 'One home for reusable images and videos.', '<button class="btn" id="media-upload-button">↥ Upload files</button>') + `<section class="panel media-library-panel"><div class="dropzone" id="dropzone"><strong>Drop images or videos here</strong><br><span class="subtle">JPG, PNG, WebP up to 10 MB · MP4 and MOV up to 100 MB</span><input id="media-input" class="hidden" type="file" multiple accept="image/jpeg,image/png,image/webp,video/mp4,video/quicktime"></div><div id="upload-queue"></div><div class="media-toolbar"><label class="media-search"><span>⌕</span><input id="resource-search" placeholder="Search files"></label><select id="media-type-filter"><option value="">All types</option><option value="image">Images</option><option value="video">Videos</option></select><select id="media-sort"><option value="newest">Newest</option><option value="oldest">Oldest</option><option value="name">Name A-Z</option><option value="largest">Largest</option></select><button class="btn secondary" id="media-select-all" type="button">Select all</button><button class="media-view-button active" data-media-view="grid" title="Grid view">▦</button><button class="media-view-button" data-media-view="list" title="List view">☷</button></div><div id="media-grid" class="media-grid">${skeleton(4)}</div></section>`;
  const input = document.querySelector('#media-input');
  document.querySelector('#media-upload-button').onclick = () => input.click();
  input.onchange = event => uploadFiles(event.target.files);
  ...
  document.querySelector('#resource-search').oninput = debounce(loadMedia);
  document.querySelector('#media-type-filter').onchange = loadMedia;
  document.querySelector('#media-sort').onchange = loadMedia;
  document.querySelector('#media-select-all').onclick = () => document.querySelectorAll('.media-select').forEach(input => { input.checked = true; });
  document.querySelectorAll('[data-media-view]').forEach(button => button.onclick = () => { document.querySelectorAll('[data-media-view]').forEach(item => item.classList.toggle('active', item === button)); document.querySelector('#media-grid').classList.toggle('media-list-view', button.dataset.mediaView === 'list'); });
  loadMedia();
}
async function loadMedia() {
  try {
    let assets = await api('media');
    const search = (document.querySelector('#resource-search')?.value || '').toLowerCase();
    const type = document.querySelector('#media-type-filter')?.value || '';
    const sort = document.querySelector('#media-sort')?.value || 'newest';
    assets = assets.filter(asset => (!search || `${asset.display_name || asset.filename} ${asset.alt_text || ''}`.toLowerCase().includes(search)) && (!type || asset.mime_type?.startsWith(`${type}/`)));
    assets.sort((left, right) => sort === 'oldest' ? left.created_at.localeCompare(right.created_at) : sort === 'name' ? (left.display_name || left.filename).localeCompare(right.display_name || right.filename) : sort === 'largest' ? right.size_bytes - left.size_bytes : right.created_at.localeCompare(left.created_at));
    document.querySelector('#media-grid').innerHTML = assets.length ? assets.map(mediaCard).join('') : '<div class="empty" style="grid-column:1/-1">No media assets found.</div>';
    document.querySelectorAll('.media-card').forEach(card => card.onclick = event => { if (!event.target.closest('.media-select')) openMediaDetails(assets.find(asset => asset.filename === card.dataset.filename)); });
  } catch (error) { document.querySelector('#media-grid').innerHTML = `<div class="empty" style="grid-column:1/-1">Media request failed: ${escapeHtml(error.message)}</div>`; }
}
function mediaCard(asset) {
  const preview = asset.mime_type?.startsWith('image/') ? `<img src="${escapeHtml(asset.public_url)}" alt="${escapeHtml(asset.alt_text || asset.display_name || asset.filename)}">` : '<span class="media-video-icon">▶</span><span>VIDEO</span>';
  return `<article class="media-card" data-filename="${escapeHtml(asset.filename)}"><div class="media-check"><input class="media-select" type="checkbox" aria-label="Select ${escapeHtml(asset.display_name || asset.filename)}"></div><div class="media-thumb">${preview}</div><div class="media-meta"><strong title="${escapeHtml(asset.display_name || asset.filename)}">${escapeHtml(asset.display_name || asset.filename)}</strong><small>${Math.round((asset.size_bytes || 0) / 1024)} KB · ${date(asset.created_at)}</small><button type="button" class="media-edit-button" title="Edit file details">✎</button></div></article>`;
}
function openMediaDetails(asset) {
  if (!asset) return;
  document.querySelector('#media-details-backdrop')?.remove(); document.querySelector('#media-details')?.remove();
  const preview = asset.mime_type?.startsWith('image/') ? `<img src="${escapeHtml(asset.public_url)}" alt="${escapeHtml(asset.alt_text || asset.display_name || asset.filename)}">` : '<div class="media-detail-video">▶ Video</div>';
  document.body.insertAdjacentHTML('beforeend', `<div class="drawer-backdrop" id="media-details-backdrop"></div><aside class="media-details" id="media-details"><div class="media-details-head"><h2>File details</h2><button class="icon-btn" id="close-media-details" title="Close">×</button></div><div class="media-detail-preview">${preview}</div><form id="media-details-form"><div class="field"><label>Filename</label><input name="display_name" value="${escapeHtml(asset.display_name || asset.filename)}" required></div><div class="field"><label>Alt text</label><input name="alt_text" value="${escapeHtml(asset.alt_text || '')}" placeholder="Describe this image for accessibility"></div><p class="media-detail-meta">${escapeHtml(asset.mime_type)} · ${Math.round((asset.size_bytes || 0) / 1024)} KB<br>Used in ${asset.usage_count || 0} place(s)</p><button type="button" class="btn secondary media-detail-action" id="copy-media-url">▣ Copy URL</button><button type="button" class="btn secondary media-detail-action" id="replace-media-file">↥ Replace file</button><input type="file" id="replace-media-input" class="hidden" accept="image/jpeg,image/png,image/webp,video/mp4,video/quicktime"><button class="btn media-detail-action" type="submit">Save changes</button><button class="btn danger media-detail-action" type="button" id="delete-media-file">♲ Delete file</button></form></aside>`);
  const close = () => { document.querySelector('#media-details-backdrop')?.remove(); document.querySelector('#media-details')?.remove(); };
  document.querySelector('#close-media-details').onclick = close; document.querySelector('#media-details-backdrop').onclick = close;
  document.querySelector('#copy-media-url').onclick = async () => { await navigator.clipboard?.writeText(asset.public_url); toast('Media URL copied.'); };
  document.querySelector('#media-details-form').onsubmit = async event => { event.preventDefault(); try { await api(`media/${encodeURIComponent(asset.filename)}`, { method: 'PATCH', body: Object.fromEntries(new FormData(event.target)) }); toast('Media details saved.'); close(); loadMedia(); } catch (error) { toast(`Save failed: ${error.message}`); } };
  document.querySelector('#delete-media-file').onclick = async () => { if (!confirm('Delete this media file? Existing content using this URL may stop displaying.')) return; try { await api(`media/${encodeURIComponent(asset.filename)}`, { method: 'DELETE' }); toast('Media file deleted.'); close(); loadMedia(); } catch (error) { toast(`Delete failed: ${error.message}`); } };
  document.querySelector('#replace-media-file').onclick = () => document.querySelector('#replace-media-input').click();
  document.querySelector('#replace-media-input').onchange = event => { if (event.target.files?.[0]) uploadFiles(event.target.files, asset.filename, close); };
}
function uploadFiles(files, replaceFilename = '', onComplete = () => {}) { Array.from(files).forEach(file => { const isImage = ['image/jpeg', 'image/png', 'image/webp'].includes(file.type); const isVideo = ['video/mp4', 'video/quicktime'].includes(file.type); const limit = isVideo ? 100 * 1024 * 1024 : 10 * 1024 * 1024; if ((!isImage && !isVideo) || file.size > limit) { toast(`${file.name}: unsupported type or file is over the size limit.`); return; } const queue = document.querySelector('#upload-queue'); const item = document.createElement('div'); item.className = 'panel-body'; item.innerHTML = `<strong>${escapeHtml(file.name)}</strong><div class="progress"><i style="width:0%"></i></div><small class="subtle">Starting upload…</small>`; if (queue) queue.appendChild(item); const xhr = new XMLHttpRequest(); xhr.open('POST', `${apiBase}media${replaceFilename ? `/${encodeURIComponent(replaceFilename)}` : ''}`); xhr.setRequestHeader('X-CSRF-Token', state.csrf); xhr.upload.onprogress = event => { if (!event.lengthComputable || !item.querySelector('i')) return; const percent = Math.round(event.loaded / event.total * 100); item.querySelector('i').style.width = `${percent}%`; item.querySelector('small').textContent = `Uploading… ${percent}%`; }; xhr.onload = () => { let result = {}; try { result = JSON.parse(xhr.responseText); } catch (error) {} if (xhr.status >= 200 && xhr.status < 300) { if (item.querySelector('small')) item.querySelector('small').textContent = replaceFilename ? 'Replaced successfully' : 'Uploaded successfully'; onComplete(); loadMedia(); } else if (item.querySelector('small')) { item.querySelector('small').textContent = result.error || 'Upload failed. Check storage permissions.'; item.querySelector('small').style.color = 'var(--danger)'; } }; xhr.onerror = () => { if (item.querySelector('small')) { item.querySelector('small').textContent = 'Upload failed: network error.'; item.querySelector('small').style.color = 'var(--danger)'; } }; const body = new FormData(); body.append('file', file); xhr.send(body); }); }
